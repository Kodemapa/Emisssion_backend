...
def generate_plot_image(city_name, year):
    ...
    try:
        ...
        plt.title(f'Year {year} - {city_name} ({state})', fontname='Sans Serif', fontsize=16, pad=22)
        ...
        for idx, (time, traffic_volume, speed, tract_id) in enumerate(all_data):
            ...
            if idx == 0:
                ax2 = ax1.twinx()
                ax2.set_ylabel('Traffic Volume (veh/hr/ln)', fontname='Sans Serif', fontsize=16, labelpad=20)
